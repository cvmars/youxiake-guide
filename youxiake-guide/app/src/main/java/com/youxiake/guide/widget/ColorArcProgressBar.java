package com.youxiake.guide.widget;

/**
 * Created by Cvmars for LiTing  on 2017/6/19
 */
...
public class ColorArcProgressBar extends View{
    ...
    private int mWidth;
    private int mHeight;
    private int diameter = getScreenWidth() / 4;  //直径
    ...
    private void initView() {
        ...
        allArcPaint.setColor(Color.parseColor("#9fe5ff"));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isNeedContent) {
            canvas.drawText(df.format(curValues)+"分", centerX, centerY + textSize / 3, vTextPaint);
        }
        if (isNeedUnit) {
            canvas.drawText(hintString, centerX, centerY + 2 * textSize / 3 + DensityUtils.dp2px(getContext(),40), hintPaint);
        }
        if (isNeedTitle) {
